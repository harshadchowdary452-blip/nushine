...
export default function DoctorAvailability() {
  ...
  const bulkUpdateMutation = useMutation({
    mutationFn: (schedulesData: Partial<DoctorWorkingHour>[]) =>
      doctorWorkingHoursApi.bulkUpdate(doctorId, { schedules: schedulesData }),
    onSuccess: () => {
  ...
  const updateOverrideMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<DoctorAvailability> }) =>
      doctorAvailabilityApi.update(doctorId, id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["doctorAvailability", doctorId] })
  ...
  const createLeaveMutation = useMutation({
    ...
    onError: () =>
      addToast({ title: "Error", description: "Failed to create leave", variant: "destructive" }),
  })
  ...
  const deleteLeaveMutation = useMutation({
    ...
    onError: () =>
      addToast({ title: "Error", description: "Failed to delete leave", variant: "destructive" }),
  })
                ...
                {overridesQuery.data?.map((ov) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Badge
                        variant={ov.is_available ? "success" : "destructive"}
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select
                value={overrideForm.is_available}
                onValueChange={(v) => setOverrideForm({ ...overrideForm, is_available: v })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      type="time"
                      value={overrideForm.lunch_start}
                      onChange={(e) =>
                        setOverrideForm({ ...overrideForm, lunch_start: e.target.value })
                      }
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      type="time"
                      value={overrideForm.lunch_end}
                      onChange={(e) =>
                        setOverrideForm({ ...overrideForm, lunch_end: e.target.value })
                      }
                    />
                  </div>
                </div>
      ...
      >
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => createLeaveMutation.mutate(leaveForm)}
              disabled={
                !leaveForm.start_date || !leaveForm.end_date || createLeaveMutation.isPending
              }
            >
              Submit Request
            </Button>
