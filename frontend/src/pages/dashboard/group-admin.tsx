...
import { Building2, Stethoscope, Users, DollarSign, TrendingUp, Sparkles, Hospital, Award } from "lucide-react"
  ...
  return (
    <motion.div className="space-y-8 relative" variants={container} initial="hidden" animate="show">
      ...
      <motion.div variants={container} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard icon={Stethoscope} title="Doctors" value={formatIndianNumber(stats?.total_doctors ?? 0)} color="primary" delay={0.05} />
        <KpiCard icon={Users} title="Patients" value={formatIndianNumber(stats?.total_patients ?? 0)} color="success" delay={0.1} />
        <KpiCard icon={DollarSign} title="Revenue" value={stats?.total_revenue != null ? formatIndianRupees(stats.total_revenue) : "₹0"} color="warning" delay={0.15}
