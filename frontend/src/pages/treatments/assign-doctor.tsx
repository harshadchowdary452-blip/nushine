...
export default function AssignDoctor() {
  ...
  const doctorList: DoctorOption[] = useMemo(() => {
    ...
    return raw.map((d: DoctorListItem): DoctorOption => ({ id: d.id, name: d.full_name || d.name || d.email || "", specialization: d.specialization || null }))
  }, [doctors])

  const assignMutation = useMutation({
    mutationFn: (data: { doctor_ids: Record<string, string> }) => {
      const assignmentList = Object.entries(data.doctor_ids).map(([item_id, assigned_doctor_id]) => ({
        item_id,
        assigned_doctor_id,
      }))
    ...
    onError: (err: Error) => addToast({ title: "Error", description: extractDetail(err) || "Failed", variant: "destructive" }),
  })
  ...
  const allAssigned = itemList.every((item: TreatmentPlanItem) => assignments[item.id] || item.assigned_doctor_id)
  const totalCost = itemList.reduce((sum: number, item: TreatmentPlanItem) => sum + (item.estimated_cost || 0), 0)

  return (
    <div className="space-y-6">
      <PageHeader title="Assign Doctors" description={(caseData as Case).case_number || caseId!.slice(0, 8)}>
      ...
      <Card>
        ...
        <CardContent className="py-2 space-y-3">
          {itemList.map((item: TreatmentPlanItem) => (
            <div key={item.id} className="flex items-center gap-4 rounded-lg border p-4">
              ...
              <div className="w-64">
                ...
                >
                  <SelectTrigger className={cn(!assignments[item.id] && !item.assigned_doctor_id && "border-yellow-300")}>
          ...
          {itemList.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-6">No procedure items to assign</p>
          )}
        </CardContent>
      </Card>
      ...
      <div className="flex items-center justify-between rounded-lg border bg-white p-4">
        <div className="text-sm text-muted-foreground">
          {Object.values(assignments).filter(Boolean).length + itemList.filter((i: TreatmentPlanItem) => i.assigned_doctor_id).length} / {itemList.length} procedures assigned · Total Cost: ₹{totalCost.toLocaleString()}
        </div>
        <Button onClick={() => setConfirmDialogOpen(true)} disabled={!allAssigned || assignMutation.isPending}>
          {assignMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : <CheckCircle2 className="h-4 w-4 mr-1" />}
      ...
      <Dialog open={confirmDialogOpen} onOpenChange={setConfirmDialogOpen}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">
            Assigning doctors to all {itemList.length} procedures. This will save the assignments and you can proceed to approval.
          </p>
          <DialogFooter>
            <Button variant="outline" onClick={() => setConfirmDialogOpen(false)}>Cancel</Button>
            <Button onClick={() => assignMutation.mutate({ doctor_ids: assignments })} disabled={assignMutation.isPending}>
              {assignMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : null} Confirm
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
