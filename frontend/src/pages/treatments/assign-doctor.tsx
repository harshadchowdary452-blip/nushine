...
export default function AssignDoctor() {
  ...
  const allAssigned = itemList.every((item: any) => assignments[item.id] || item.assigned_doctor_id)
  const totalCost = itemList.reduce((sum: number, item: any) => sum + (item.estimated_cost || 0), 0)

  return (
    <div className="space-y-6">
      <PageHeader title="Assign Doctors" description={(caseData as any).case_number || caseId!.slice(0, 8)}>
      ...
      <div className="flex items-center justify-between rounded-lg border bg-white p-4">
        <div className="text-sm text-muted-foreground">
          {Object.values(assignments).filter(Boolean).length + itemList.filter((i: any) => i.assigned_doctor_id).length} / {itemList.length} procedures assigned · Total Cost: ₹{totalCost.toLocaleString()}
        </div>
