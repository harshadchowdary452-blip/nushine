    ...
    return (
      <div className="p-6 flex gap-3">
        {Array.from({ length: 7 }).map((_, i) => (
          <Skeleton key={i} className="flex-1 h-64" />
        ))}
      </div>
    )
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Visual overview of all treatments by status"
        actions={
          <Button variant="outline" size="sm" onClick={() => navigate("/treatments")}>
            List View
          </Button>
        }
      />
        ...
        {columns.map((col) => (
          <div key={col.key} className="flex-1 min-w-[220px] flex flex-col">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs text-muted-foreground ml-auto bg-muted rounded-full px-1.5">
                {col.items.length}
              </span>
            </div>
