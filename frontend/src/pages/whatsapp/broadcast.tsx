...
export default function WhatsAppBroadcast() {
  ...
  const [preview, setPreview] = useState<
    Array<{ name: string; phone: string; message: string; link: string }>
  >([])
  ...
  const filteredPatients = useMemo(() => {
    ...
    return allPatients.filter(
      (p) => (p.patient_name || "").toLowerCase().includes(q) || (p.phone || "").includes(q),
    )
  ...
  const filteredLeads = useMemo(() => {
    ...
    const q = searchTerm.toLowerCase()
    return allLeads.filter((l) => l.lead_name.toLowerCase().includes(q) || l.mobile.includes(q))
  }, [allLeads, searchTerm])

  function resolveAudience(): Array<Patient | Lead> {
    switch (audienceType) {
      case "all_patients":
        return allPatients.filter((p) => p.phone)
      case "all_leads":
        return allLeads.filter((l) => l.mobile)
      case "appointment_tomorrow":
        return allPatients.filter(
          (p) => p.phone && (p as unknown as Record<string, unknown>).next_appointment_date,
        )
      case "pending_followups":
        return allPatients.filter(
          (p) => p.phone && (p as unknown as Record<string, unknown>).next_follow_up,
        )
      case "six_month_recall":
        return allPatients.filter((p) => p.phone)
      default:
        return []
    }
  ...
  function fillTemplate(
    msg: string,
    recipient: { patient_name?: string | null; lead_name?: string; full_name?: string },
  ): string {
  ...
  function getName(r: Patient | Lead): string {
    ...
    if (patient.patient_name || patient.full_name)
      return patient.patient_name || patient.full_name || "-"
  ...
  function handleGenerate() {
    ...
    if (audience.length === 0) {
      addToast({
        title: "No recipients",
        description: "No matching patients/leads found",
        variant: "destructive",
      })
      return
    }
    if (!message.trim()) {
      addToast({
        title: "No message",
        description: "Select a template or write a custom message",
        variant: "destructive",
      })
      return
    }
    ...
    addToast({
      ...
      description: `${links.length} WhatsApp links created`,
      variant: "success",
    })
  }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <PageHeader
        ...
        description="Generate WhatsApp messages for bulk communication"
        actions={
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-sm">
                <Filter className="h-4 w-4 text-primary" /> Select Audience
              </CardTitle>
            </CardHeader>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Button variant="outline" onClick={handlePreview} disabled={!getMessage()}>
                <ChevronRight className="h-4 w-4 mr-1" /> Preview (
                {resolveAudience().slice(0, 10).length} samples)
              </Button>
            </CardContent>
          </Card>
        </div>
                  ...
                  <div key={i} className="rounded-lg border border-gray-100 p-2 text-xs">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <a
                        ...
                        className="text-[var(--ds-primary)] hover:underline flex items-center gap-0.5"
                      >
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm font-medium">
              Preview ({preview.length} samples)
            </CardTitle>
          </CardHeader>
