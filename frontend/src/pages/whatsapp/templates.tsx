...
const DEFAULT_TEMPLATES = [
  ...
  {
    name: "Treatment Completion Thank You",
    message:
...
export default function WhatsAppTemplates() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: { name: string; message: string } }) =>
      whatsappTemplatesApi.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["whatsapp-templates"] })
  ...
  function handleSave() {
    if (!name.trim() || !message.trim()) {
      addToast({
        title: "Validation",
        description: "Name and message required",
        variant: "destructive",
      })
      return
    }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <PageHeader
        ...
        description="Manage message templates for WhatsApp communication"
        actions={
            ...
            {allTemplates.map((t: WATemplate) => (
              <Card key={t.id} className="border-gray-200">
                ...
                <CardContent>
                  ...
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon-sm" onClick={() => openCreate(t)}>
                      <Edit3 className="h-3.5 w-3.5" />
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon-sm"
                      onClick={() => copyToClipboard(t.message, t.id)}
                    >
                      {copiedId === t.id ? (
