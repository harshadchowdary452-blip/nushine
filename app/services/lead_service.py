...
class LeadService:
    ...
    async def convert(self, lead_id: str, data: dict, user_id: str = None) -> dict:
        ...
        phone = data.get("phone") or lead.mobile
        # Check for existing patient by phone + hospital
        existing = (await self.db.execute(
            select(Patient).where(
                Patient.phone == phone,
                Patient.hospital_id == lead.hospital_id,
            ).limit(1)
        )).scalar_one_or_none()
        async with self.db.begin_nested():
            if existing:
                ...
                patient = Patient(
                    full_name=data.get("patient_name") or lead.lead_name,
                    ...
                    medical_history=data.get("notes") or lead.notes,
                )
                self.db.add(patient)
                await self.db.flush()
